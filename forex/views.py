...
def options(request, symbol, account):
    ...
    all_trades = OptionTrade.objects.filter(trader=trading_account).order_by('-id')
    closed_trades = OptionTrade.objects.filter(trader=trading_account, expired=True).order_by('-id')[:5]
...
def update_trade_outcomes(request):
    ...
    for demo_trade in demo_trades:
        ...
        try:
            forex_pair = ForexPair.objects.get(symbol=demo_trade.symbol)
